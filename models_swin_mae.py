...
from monai.utils import ensure_tuple_rep, optional_import,look_up_option
...
class MaskedAutoencoderSwinViT(nn.Module):
    """ Masked Autoencoder with Swin VisionTransformer backbone
    """
    def __init__(self, img_size=224, patch_size=16, in_chans=6,
                 embed_dim=1024, depth=24, num_heads=16,
                 decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
                 mlp_ratio=4., norm_layer=nn.LayerNorm, norm_pix_loss=False,drop_path_rate=0.0,depths=(2, 2, 2, 2),downsample="merging"):
        super().__init__()
    ...
    def forward_encoder(self, x, mask_ratio):
        ...
        # x0 = self.pos_drop(x)
        x0_out = self.proj_out(x, True)
        x1 = self.layers1[0](x.contiguous())
        x1_out = self.proj_out(x1, True)
        x2 = self.layers2[0](x1.contiguous())
        x2_out = self.proj_out(x2, True)
        x3 = self.layers3[0](x2.contiguous())
        x3_out = self.proj_out(x3, True)
        x4 = self.layers4[0](x3.contiguous())
        x4_out = self.proj_out(x4, True)
        return x4_out, mask, ids_restore
